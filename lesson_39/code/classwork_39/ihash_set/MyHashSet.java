...
public class MyHashSet<E> implements ISet<E> {
    ...
    public boolean addElement(E element) {
        ...
        if (hashset[index].contains(element)) { // лезем в корзину
    ...
    public boolean contains(Object o) {
        ...
        return hashset[index].contains(o); // проверяем наличие элемента, contains - это метод из LinkedList???
    }
    ...
    public boolean remove(Object o) {
        ...
        boolean res = hashset[index].remove(o); // метод remove - это метод из LinkedList???
        if(res){
            size--;
        }
        return res;
