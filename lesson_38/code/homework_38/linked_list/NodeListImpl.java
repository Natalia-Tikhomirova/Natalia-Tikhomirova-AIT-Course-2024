package homework_38.linked_list;
...
//                }
    ...
    public int lastIndexOf(Object o) {
        // TODO - самостоятельно
        // Перебираем список с конца к началу:
    ...
    // удаляет node из списка: first <-> node <-> last, обновляя ссылки на соседние узлы
    private E unlink(Node<E> node) {
        E victim = node.data; // хранит данные удаляемого узла
        Node<E> prev = node.prev; // ссылка на предыдущий узел относительно удаляемого
        Node<E> next = node.next; //ссылка на следующий узел относительно удаляемого
       // Обновление ссылки предыдущего узла:
        if (prev != null) {
            prev.next = next;
            node.prev = null;
        } else {
        ...
        // Обновление ссылки следующего узла:
        if (next != null) {
            next.prev = prev;
            node.next = null;
        } else {
        ...
        return victim; // возврат значения удаленного узла
    }
